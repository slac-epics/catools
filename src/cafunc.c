    ...
    if (pchan->state == cs_conn)
    {
        ...
        else
        {
            ...
            while (pchan->error == CA_WAIT)
            {
                ...
                if (count > imax)
                {
                    ca_execerror("ca_get_real_value timeout on:", (char *) ca_name(pchan->chid));
                    break;
                }
                ca_pend_event(CA.PEND_EVENT_TIME);
            }
    ...
    if (pchan->state == cs_conn)
    {
        ...
        else
        {
            ...
            while (pchan->error == CA_WAIT)
            {
                ...
                if (count > imax)
                {
                    ca_execerror("ca_get_string timeout on:", (char *) ca_name(pchan->chid));
                    break;
                }
                ca_pend_event(CA.PEND_EVENT_TIME);
            }
...
/* populate hash table here return the address */
    ...
    if (name)
        ...
        pchandata = (chandata *) ca_check_hash_table(" ");
 ...
 **************************************************/
int
ca_get_conn_data(dbr_type, pchandata)
     int dbr_type;
     chandata *pchandata;
{
    int status;
...
**************************************************/
int
ca_get_dbr_sts_string(pchandata)
     chandata *pchandata;
{
    int status;
...
**************************************************/
int
ca_get_dbr_sts_float(pchandata)
     chandata *pchandata;
{
    int status;
...
**************************************************/
int
ca_get_dbr_sts_double(pchandata)
     chandata *pchandata;
{
    int status;
...
int
ca_get_dbr_gr_enum2(pchandata, dbr)
...
**************************************************/
int
ca_get_dbr_ctrl_double(pchandata)
     chandata *pchandata;
{
    int status;
    ...
    if (pchandata->state == cs_conn)
    {
        ...
        else
        {
            ...
            if (status == ECA_NORMAL)
            {
                ...
                if (pchandata->largest < dbr.value)
                    pchandata->largest = (float) dbr.value;
                if (pchandata->smallest > dbr.value)
                    pchandata->smallest = (float) dbr.value;
            }
        }
        return status;
    }
 ...
 ********************************************************/
int
ca_put_conn_data(dbr_type, pchandata, pvalue)
     int dbr_type;
     chandata *pchandata;
     void *pvalue;
{
 ...
 ********************************************************/
int
ca_put_dbr_float(pchandata, pvalue)
     chandata *pchandata;
     void *pvalue;
{
    int status;
 ...
 ********************************************************/
int
ca_put_dbr_double(pchandata, pvalue)
     chandata *pchandata;
     void *pvalue;
{
    int status;
    ...
    if (pchandata->state == cs_conn)
    {
        ...
        if (status != ECA_NORMAL)
        {
            ...
            pchandata->setpoint = (float) value;
 ...
 ********************************************************/
int
ca_put_dbr_string(pchandata, pvalue)
     chandata *pchandata;
     void *pvalue;
{
    int status;
    ...
    if (pchandata->state == cs_conn)
    {
        ...
        if (status != ECA_NORMAL)
        {
            ...
            pchandata->setpoint = (float) atof(pchandata->string);
 ...
 *************************************************/
int
ca_pvlist_search(noName, pvName, list)
     int noName;
     char **pvName;
     chandata **list;
{
    ...
    {
        if (pvName[i])
            pchan = (chandata *) ca_check_hash_table(pvName[i]);
        else
            pchan = (chandata *) ca_check_hash_table(" ");
        pnow->next = pchan;
        pchan->next = NULL;
        pnow = pchan;
    }
 ...
 *************************************************/
void
ca_get_string_array(noName, pvName, value)
     int noName;
     char **pvName;
     char **value;
{
    ...
    while (snode)
    {
        ...
        if (pchan->state == cs_conn && pchan->type != TYPENOTCONN)
        {
            ...
            while (pchan->error == CA_WAIT)
            {
                ...
                if (count > imax)
                {
                    ca_execerror("ca_get_string_array timeout on:", (char *) ca_name(pchan->chid));
                    break;
                }
                ca_pend_event(CA.PEND_EVENT_TIME);
            }
 ...
 *************************************************/
void
ca_get_all_string_array(noName, pvName, value)
     int noName;
     char **pvName;
     char **value;
{
    ...
    while (snode)
    {
        ...
        if (pchan->state == cs_conn && pchan->type != TYPENOTCONN)
        {
            ...
            sprintf(pchan->string, "%-20s %2d %2d", pchan->string, pchan->status, pchan->severity);
            *(value + i) = pchan->string;
        }
        snode = snode->next;
        i++;
 ...
 *************************************************/
void
ca_get_info_array(noName, pvName, value)
     int noName;
     char **pvName;
     double *value;
{
    ...
    while (snode)
    {
        ...
        if (pchan->state == cs_conn && pchan->type != TYPENOTCONN)
        {
            ...
            while (pchan->error == CA_WAIT)
            {
                ...
                if (count > imax)
                {
                    ca_execerror("ca_get_info_array timeout on:", (char *) ca_name(pchan->chid));
                    break;
                }
                ca_pend_event(CA.PEND_EVENT_TIME);
            }
 ...
 *************************************************/
void
ca_populate_info_array(noName, pvName)
     int noName;
     char **pvName;
{
    int i, status, command_error = 0;
    ...
    while (snode)
    {
        ...
        if (pchan->state == cs_conn && pchan->type != TYPENOTCONN)
        {
            ...
            while (pchan->error == CA_WAIT)
            {
                ...
                if (count > imax)
                {
                    ca_execerror("ca_populate_info_array timeout", (char *) ca_name(pchan->chid));
                    break;
                }
                ca_pend_event(CA.PEND_EVENT_TIME);
            }
 ...
 *************************************************/
void
ca_get_error_array(noName, pvName, value)
     int noName;
     char **pvName;
     int *value;
{
 ...
 ****************************************************/
void
ca_get_field_type_array(noName, pvName, value)
     int noName;
     char **pvName;
     int *value;
{
 ...
 ****************************************************/
void
ca_get_element_count_array(noName, pvName, value)
     int noName;
     char **pvName;
     int *value;
{
 ...
 ****************************************************/
void
ca_old_status_array(noName, pvName, value)
     int noName;
     char **pvName;
     int *value;
{
 ...
 *************************************************/
void
ca_get_status_array(noName, pvName, value)
     int noName;
     char **pvName;
     int *value;
{
    ...
    while (snode)
    {
        ...
        if (pchan->state == cs_conn && pchan->type != TYPENOTCONN)
        {
            ...
            while (pchan->error == CA_WAIT)
            {
                ...
                if (count > imax)
                {
                    ca_execerror("ca_get_status_array timeout on:", (char *) ca_name(pchan->chid));
                    break;
                }
                ca_pend_event(CA.PEND_EVENT_TIME);
            }
 ...
 *************************************************/
void
ca_get_double_array2(noName, pvName, value)
     int noName;
     char **pvName;
     double *value;
{
 ...
 *************************************************/
void
ca_get_all_double_array(noName, pvName, value)
     int noName;
     char **pvName;
     double *value;
{
 ...
 *************************************************/
void
ca_get_double_array(noName, pvName, value)
     int noName;
     char **pvName;
     double *value;
{
    ...
    while (snode)
    {
        ...
        if (pchan->state == cs_conn && pchan->type != TYPENOTCONN)
        {
            ...
            while (pchan->error == CA_WAIT)
            {
                ...
                if (count > imax)
                {
                    ca_execerror("ca_get_double_array timeout on:", (char *) ca_name(pchan->chid));
                    break;
                }
                ca_pend_event(CA.PEND_EVENT_TIME);
            }
 ...
 *************************************************************/
void
ca_put_string_array(noName, pvName, value)
     int noName;
     char **pvName;
     char **value;
{
    ...
    while (snode)
    {
        ...
        if (status != ECA_NORMAL)
        {
            ca_execerror("ca_put failed on", (char *) ca_name(pchan->chid));
            pchandata = pchan;
            ca_check_return_code(status);
        }
        else
            pchan->setpoint = (float) atof(value[i]);
 ...
 *************************************************************/
void
ca_put_double_array(noName, pvName, value)
     int noName;
     char **pvName;
     double *value;
{
 ...
 *****************************************/
int
ca_get_native_wave_form(name, count, pdata)
     char *name;
     int count;
     void *pdata;
{
 ...
 *****************************************/
int
ca_put_native_wave_form(name, n, val)
     char *name;
     int n;
     void *val;
 ...
 *****************************************/
int
ca_put_string_wave_form(name, n, val)
     char *name;
     int n;
     char **val;
{
    ...
    if (pchan->type != TYPENOTCONN && pchan->state != cs_closed)
    {
        ...
        case DBR_CHAR:
            for (i = 0; i < n; i++)
                wf_buf.cv[i] = atoi(val[i]);
            status = ca_array_put(type, count, pchan->chid, wf_buf.cv);
            break;
        case DBR_STRING:
            for (i = 0; i < n; i++)
                sprintf(wf_buf.pv[i], "%s", val[i]);
            status = ca_array_put(type, count, pchan->chid, wf_buf.pv);
            break;
        case DBR_FLOAT:
            for (i = 0; i < n; i++)
                wf_buf.fv[i] = (float) atof(val[i]);
            status = ca_array_put(type, count, pchan->chid, wf_buf.fv);
            break;
        case DBR_DOUBLE:
            for (i = 0; i < n; i++)
                wf_buf.dv[i] = atof(val[i]);
            status = ca_array_put(type, count, pchan->chid, wf_buf.dv);
            break;
        case DBR_SHORT:
        case DBR_ENUM:
            for (i = 0; i < n; i++)
                wf_buf.sv[i] = atoi(val[i]);
            status = ca_array_put(type, count, pchan->chid, wf_buf.sv);
            break;
        default:
            for (i = 0; i < n; i++)
                wf_buf.iv[i] = atoi(val[i]);
            status = ca_array_put(type, count, pchan->chid, wf_buf.iv);
            break;
        }
        if (status != ECA_NORMAL)
